...
class Hill
  ...
  def add_path_from_hill_if_accessible(hill)
    return unless @elevation <= hill.elevation + 1

    @accessible_hills << hill
  end

  def shortest_distance_to(heightmap, destination_sigil)
    distance_to = { self => 0 }
    unvisited_hills = []
    ...
    distance_to.select { |hill| hill.sigil == destination_sigil }.values.min
  end
end
...
heightmap.each_with_index do |hills, y|
  hills.each_with_index do |hill, x|
    ...
    hill.add_path_from_hill_if_accessible(heightmap[y - 1][x]) if y > 0
    hill.add_path_from_hill_if_accessible(heightmap[y + 1][x]) if y < heightmap.length - 1
    hill.add_path_from_hill_if_accessible(heightmap[y][x - 1]) if x > 0
    hill.add_path_from_hill_if_accessible(heightmap[y][x + 1]) if x < hills.length - 1
  end
end

puts end_point.shortest_distance_to(heightmap, 'a')
